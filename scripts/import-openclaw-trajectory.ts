/**
 * Import a third-party CLI trajectory (.trajectory.jsonl) into Drift fixture format.
 *
 * a third-party CLI stores session trajectories in:
...
function convertToFixture(
  ...
  return {
    ...
    description: `Imported a third-party CLI trajectory: "${goalPrompt.slice(0, 60)}"`,
    agent: agentId,
    ...
    label: {
      ...
      annotator_notes: `Imported from a third-party CLI trajectory. ${totalToolCalls} tool calls across ${allPrompts.length} turns (${allPrompts.length} user interactions injected). Goal (turn #${goalIndex}): "${goalPrompt.slice(0, 80)}". [ANNOTATE: review and label]`,
...
a third-party CLI trajectory locations:
...
`)
  ...
  const prompts = turns.map(t => t.prompt).filter(Boolean)
  console.log(`
✓ Imported a third-party CLI trajectory: ${outputFile}
